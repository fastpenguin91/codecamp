...
/*
  ...
  1 * 2 * 3 * 4 * 5 * 6 * 7 = 5040 / 7 = 720

  1 * 2 * 3 * 4 * 5 * 6  = 720 / 6 = 120
...
function allPoss(str){
  ...
  let numOfArrs = factorial(str.length);
  let strList = [];
...
//      console.log("working on element: " + i2);

      // create the string that potentially replaces the previous string.
      let testStr = strList[i2].slice(0, i) + strList[i2].slice(i) + str[i];

      console.log(str[i] + " should exist " + (strList.length / str.length) + " times at index " + i );
      // third loop? to make a timesToExist...
